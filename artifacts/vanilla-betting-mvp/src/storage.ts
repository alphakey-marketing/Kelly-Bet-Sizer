...
/**
 * Marks a bet as won or lost.
 * Calculates P&L, updates the bet record, and adjusts the bankroll.
 * Returns the new bankroll total.
 */
export function resolveBet(id: string, outcome: "won" | "lost"): number {
  const bets  = loadBets();
  ...
  // Win = net profit on stake. Lose = lose the stake.
  const pnl = outcome === "won" ? stake * (bet.decimalOdds - 1) : -stake;
